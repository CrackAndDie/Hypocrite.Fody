...
namespace Hypocrite.Fody.AttributeWeavers
{
	public static class NotifyWeaver
	{
			...
			List<string> dependentProperties)
		{
			...
			{
				logger?.Invoke($"Property {property.DeclaringType.FullName}.{property.Name} has no setter, therefore it is not possible for the property to change, and thus should not be marked with [Notify]");
				return;
			}
